...
function salaryRangeCalculator(brutoSalary, months) {
    ...
    if (brutoSalary >= 9000000) {
        ...
        temp = brutoSalary - tax
        bpjs = temp * 0.05
        netSalary = brutoSalary - tax - bpjs
        totalBPJS = bpjs * months
    }
    else if (brutoSalary >= 6000000 && brutoSalary <= 8999999) {
        tax = brutoSalary * 0.03
        temp = brutoSalary - tax
        bpjs = temp * 0.04
        netSalary = brutoSalary - tax - bpjs
        totalBPJS = bpjs * months
    ...
    // console.log(tax,bpjs);
    
    return console.log('netSalary = '+netSalary+', totalBPJS yang telah dibayarkan = '+totalBPJS)
}
