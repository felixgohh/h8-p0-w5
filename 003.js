...
function cart(shops) {
    // your code here 
    if (shops.length === 0){
        return 'anda harus memilih produk'
    }
    ...
    for(var i = 0; i < products.length; i++){
        ...
        for (var j = 0; j < shops.length; j++){
            if (object.id === shops[j]){
                object.qty += 1
                object.subtotal += products[i].price
                result.total += products[i].price
            }
        }

        if (object.qty !== 0){
            result.carts.push(object)
        }
    }
    return result
}

// console.log(cariIndex(2));
